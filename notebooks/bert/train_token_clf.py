...
def tokenize_and_align_labels(examples):
    ...
    for i, label in enumerate(examples["ner_tags"]):
        ...
        for word_idx in word_ids:
            if word_idx is None:
                label_ids.append(-100)  # Special tokens are marked with -100.
            else:
                # Check if word_idx is within the range of labels, if not, append -100.
...
# Use the modified tokenize_and_align_labels function
tokenized_train = train.map(tokenize_and_align_labels, batched=True)
tokenized_eval = eval.map(tokenize_and_align_labels, batched=True)
...
from lib import format_tokenclf_result
token_classifier = pipeline(
    "token-classification", model=model, tokenizer=tokenizer, aggregation_strategy="simple"
)

ret = token_classifier('Search for emails from imsrilathaa with the subject swab result on October nineteenth')
